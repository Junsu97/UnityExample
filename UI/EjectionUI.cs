...
{
   [SerializeField] private Text ejectionResultText;
   [SerializeField] private Image ejectionPlayer;
   ...
   public void Open(bool isEjection, EPlayerColor ejectionPlayerColor, bool isImposter, int remainImposterCount)
   {
      ...
      StartCoroutine(ShowEjectionResult_Coroutin(ejectPlayer, text));
   }

   private IEnumerator ShowEjectionResult_Coroutin(InGameCharacterMover ejectionPlayerMover, string text)
   {
      ...
      if (ejectionPlayerMover != null)
      {
         ...
         while (timer <= 1f)
         {
            ...
            ejectionPlayer.rectTransform.anchoredPosition =
               Vector2.Lerp(left.anchoredPosition, right.anchoredPosition, timer);
      ...
      while (backText.Length != 0)
      {
         ...
         ejectionResultText.text =
            string.Format("<color=#FFFFFF>{0}</color><color=#000000>{1}</color>", forwardText, backText);
